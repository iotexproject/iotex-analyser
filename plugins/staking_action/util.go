...
func getBucketSumAmountByBucketID(tx *gorm.DB, bucketID uint64) (decimal.Decimal, error) {
	...
	if err := tx.Model(&models.StakingAction{}).Select("sum(amount)").Where("bucket_id=?", bucketID).Scan(&amount).Error; err != nil {
		return zero, err
	}
	if amount.String == "" {
		return zero, nil
...
func getBucketOwnerWithHeight(bucketID, height uint64) (string, error) {
	...
	db := db.DB()
	if err := db.Model(&models.StakingAction{}).Select("owner_address").Where("block_height<=? and bucket_id=?", height, bucketID).Order("id desc").Limit(1).Scan(&addr).Error; err != nil {
		return "", err
	}
