...
func (b stakingActionChPlugin) getBucketSumAmountFromCacheByBucketID(bucketID uint64) (decimal.Decimal, error) {
	...
	if err := chDB.Model(&StakingActions{}).Select("sum(amount)").Where("bucket_id=?", bucketID).Scan(&amount).Error; err != nil {
		return zero, err
	}
...
func (b stakingActionChPlugin) getFixBucketSumAmountFromCacheByBucketID(bucketID uint64) (decimal.Decimal, error) {
	...
	if err := chDB.Model(&StakingActions{}).Select("sum(amount)").Where("bucket_id=? and act_type<>'Unstake'", bucketID).Scan(&amount).Error; err != nil {
		return zero, err
	}
...
func (b stakingActionChPlugin) getBucketInfoAddressFromCacheByBucketID(bucketID uint64) (*BucketInfo, error) {
	...
	if err := chDB.Model(&StakingActions{}).Select("owner_address,candidate,auto_stake,duration").Where("bucket_id=? and amount > 0", bucketID).Order("block_height, index desc").Limit(1).Scan(&bi).Error; err != nil {
		return nil, err
	}
	return &bi, nil
}
