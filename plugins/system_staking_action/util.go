...
func getTokenSumAmountByTokenID(tx *gorm.DB, tokenID uint64) (decimal.Decimal, error) {
	var amount sql.NullString
	zero := decimal.NewFromInt(0)
	if err := tx.Model(&models.SystemStakingActions{}).Select("sum(amount)").Where("token_id=?", tokenID).Scan(&amount).Error; err != nil {
		return zero, err
	}
	if amount.String == "" {
		return zero, nil
...
func getTokenInfoAddressByTokenID(tx *gorm.DB, tokenID uint64) (*TokenInfo, error) {
	var bi TokenInfo
	if err := tx.Model(&models.SystemStakingActions{}).Select("owner_address,candidate,auto_stake,duration").Where("token_id=?", tokenID).Order("id desc").Limit(1).Scan(&bi).Error; err != nil {
		return nil, err
	}
	return &bi, nil
}
